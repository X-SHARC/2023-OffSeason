...
public class Elevator extends SubsystemBase {
  ...
  private double kP = 0.045;       // bak bunlara
  ...
  private double softLimit = (115*17.42*2048)/(gearCircumference*2);      // bak bunlara
  ...
  public Elevator() {
    ...
    public void elevatorUp(){
        elevatorMasterMotor.set(ControlMode.PercentOutput, limiter.calculate(0.45));  // bak bunlara
    ...
    private ElevatorLevel getLevel(){
      if(getDistance()>25 && getDistance() <70){    // bak bunlara
        return ElevatorLevel.MIDROW;
      }
      else if(getDistance()>70){     // bak bunlara
  ...
  public void periodic() {
    ...
    else if(distance>= 28) RobotState.setElevated(true);     // bak bunlara
